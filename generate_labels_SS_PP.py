...
def build_records(ws, field_idx):
    ...
    for row in rows:
        ...
        it_code = str(it).strip().upper() if it not in (None, "") else ""
        ...
        if it_code.startswith("PP"):
            f14 = str(remarks).strip() if remarks not in (None, "") else ""
        else:
